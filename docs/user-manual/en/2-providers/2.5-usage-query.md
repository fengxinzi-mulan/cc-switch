...
| **Manual Enable (built-in templates)** | Token Plan, third-party balance query, Sub2API                     | Yes (see below)      |
...
| Sub2API            | Any self-hosted Sub2API instance                          | API-key quota, subscription windows, or wallet balance |
...
4. Select the right built-in template (e.g., Token Plan, third-party balance, Sub2API) or choose "Custom"
...
CC Switch provides multiple presets. The editable script presets include:
...
Calls Sub2API's `GET /v1/usage` endpoint with `Authorization: Bearer <API Key>`. The template supports HTTP for self-hosted instances, normalizes Base URLs that already end in `/v1`, `/v1beta`, or a trailing slash, and displays expiration and reset times in the local timezone of the machine running CC Switch as `YYYY-MM-DD HH:mm`. It then displays API-key quotas and rate-limit windows, daily/weekly/monthly subscription windows, or wallet balance.

Both API Key and Base URL may be left empty to reuse the provider configuration, or filled with dedicated query credentials. After selecting the template, use **Test Script** to confirm that the deployed Sub2API version supports the endpoint.
...
2. Whether a suitable built-in template (Token Plan / third-party balance / Sub2API / custom) is selected
