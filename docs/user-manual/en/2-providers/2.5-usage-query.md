...
Calls Sub2API's `GET /v1/usage` endpoint with `Authorization: Bearer <API Key>`. The template normalizes Base URLs that already end in `/v1`, `/v1beta`, or a trailing slash, then displays API-key quotas and rate-limit windows, daily/weekly/monthly subscription windows, or wallet balance.
